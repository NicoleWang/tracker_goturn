...
class regressor:
    ...
    def estimate(self, curr_search_region_batch, target_region_batch):
        ...
        """
        assert len(curr_search_region_batch) == len(target_region_batch)
        net = self.net
        # reshape the inputs
        img_num = len(curr_search_region_batch)
        net.blobs['image'].data.reshape(img_num, self.channels, self.height, self.width)
        net.blobs['target'].data.reshape(img_num, self.channels, self.height, self.width)
        net.blobs['bbox'].data.reshape(img_num, 4, 1, 1)

        self.preprocess_batch(curr_search_region_batch, target_region_batch)
        #curr_search_region_batch = self.preprocess_batch(curr_search_region_batch)
        #target_region_batch = self.preprocess_batch(target_region)
